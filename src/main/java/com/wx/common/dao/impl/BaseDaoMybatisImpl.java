package com.wx.common.dao.impl;
...
public class BaseDaoMybatisImpl<T> extends SqlSessionDaoSupport implements BaseDao<T> {
	...
	private final String MAPPERPATH = "com.wx.common.bean.";
	...
	public T findOne(T t, String sqlId) {
		List<T> list = super.getSqlSession().selectList(MAPPERPATH + t.getClass().getSimpleName() + "Mapper." + sqlId,
				t);
		if (list != null && list.size() > 0) {
			return list.get(0);
		} else {
	...
	public double getFunc(Class<T> clazz, String sqlId, Map<String, Object> parameterMap) {
		return new Double(super.getSqlSession()
				.selectOne(MAPPERPATH + clazz.getSimpleName() + "Mapper." + sqlId, parameterMap).toString());
	}
