...
public interface BaseDao<T> {
	 ...
	 *            :要保存的数据对象
	 * @param sqlId:
	 *            mapper中的方法名 "com.yc.bean."+ 类名 +"Mapper."+sqlId
	 */
	...
	/**
	 * 批量添加<br />
	 * mysql 代码: <insert id="addTag" parameterType="Tag" > insert into
	 * tag(tid,tname,tcount) values
	 * <foreach collection="list" open="" close="" separator="," item="item">
	 * (#{item.tid},#{item.tname},1) </foreach> </insert>
	 * 
	 * @param clazz
	 * @param sqlId
	 * @param list
	 ...
	 * 批量更新指定的行 <br />
	 * <!-- 批量处理: 将标签的数量加一,更新操作 -->
	 * <update id="increaseCount" parameterType="Tag"> update tag set
	 * tcount=tcount+1 where tid in
	 * <foreach collection="list" open="(" close=")" separator="," item="tag">
	 * #{tag.tid} </foreach> </update>
	 * 
	 * @param clazz
	 * @param sqlId
	 * @param list
