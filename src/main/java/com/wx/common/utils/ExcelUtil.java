...
public class ExcelUtil {
	...
	private static <T> void fillSheet(WritableSheet sheet, List<T> list, LinkedHashMap<String, String> fieldMap,
		...
		for (int index = firstIndex; index <= lastIndex; index++) {
			...
			for (int i = 0; i < enFields.length; i++) {
				...
				if(i==1){
					if(label.getContents().toString().equals("1")){
						label.setString("男");
					}else{
						label.setString("女");
					}
				...
				if(i==5){
					long msgCreateTime = Long.parseLong(label.getContents()) * 1000L;  
	...
	public static LinkedHashMap<String, String> getLeadInFiledPublicQuestionBank() {  
	    // excel的表头与文字对应  
	    ...
	    map.put("province", "省份");
	    map.put("city", "城市");
	    map.put("subscribe_time", "关注时间");  
	    map.put("subscribe", "是否关注");
	    map.put("headimgurl", "头像");
	    return map;   
	}
}
