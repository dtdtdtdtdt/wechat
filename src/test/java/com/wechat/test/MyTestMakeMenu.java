...
//		Button button[] = new Button[2];      // 2	这种方式不行  为什么？
		...
		if( firstButton.length>0 ){ 
			...
			if( buttonList.size()>0 ){ //如果有二级菜单
				//二级菜单
				for(int i=firstButton.length;i<buttonList.size()+1;i++){
					finallyButton[i] = buttonList.get(i-1);
				}
			}
		...
		String ACCESS_TOKEN = "88LPYgwwTG9LDBvQxY06aZ71CBXXD4OgItC4QlwmjMww1mCgfQ8XsQGSQC8nW8liyj0hCxy13VzvNtTVXnWxw2CWDOrEYr7xUWup_0sgCbYGWEjADAYOW";
		//先删除菜单
...
//		System.out.println("删除菜单"+ jodel.toString() );
